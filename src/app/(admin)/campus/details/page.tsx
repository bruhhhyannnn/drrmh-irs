...
import { useCampusHeadcountPerEvent, useEvents } from '@/components/hooks/use-campus';
...
export default function CampusDetailsPage() {
  ...
  const { data: event } = useCampusHeadcountPerEvent(selectedEvent?.id ?? '');
  ...
  {
    /*if (!event?.length) return <p>No headcount for this event.</p>*/
  }
          ...
          <div className="mt-5">
            <div className="flex flex-col rounded-2xl border border-gray-200 bg-white p-4 shadow-sm dark:border-white/10 dark:bg-gray-900">
              ...
              <div className="flex flex-wrap flex-1 items-center justify-center">
                {headcountPerEventData.length > 0 &&
                totalCount > 0 &&
                      ...
                      <PieChart>
                        ...
                        >
                          {headcountPerEventData.map((_, index) => (
                            <Cell key={index} fill={colors(index, headcountPerEventData.length)} />
                          ))}
                        </Pie>
                        <Tooltip
                          contentStyle={{
                      ...
                      {headcountPerEventData.map((entry, index) => (
                        <div key={index} className="flex items-center gap-1">
                          <div
                            ...
                            style={{ backgroundColor: colors(index, headcountPerEventData.length) }}
                          />
