...
import { AlertTriangle, FileText, Users, UserX } from 'lucide-react';
...
export function EventSummary({ eventId }: { eventId?: string }) {
  ...
  const totalAffected = reports.reduce((s, r) => s + totalPopulationCount(r.population_counts), 0);
  const categoryTotals = sumPopulationCountsByCategory(reports);
        ...
        {categoryTotals.length > 0 && (
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6">
            {categoryTotals.map((t) => (
              <MiniStatCard key={t.id} label={t.name} value={t.total.toLocaleString()} />
            ))}
          </div>
        )}
      </div>
...
// ─── Mini stat card — compact secondary variant of StatCard ─
function MiniStatCard({ label, value }: { label: string; value: string | number }) {
  return (
    <div className="rounded-lg border border-gray-200 bg-white px-3 py-2 dark:border-white/5 dark:bg-white/3">
      <p className="truncate text-[11px] font-medium text-gray-500 dark:text-gray-400">{label}</p>
      <p className="text-base font-semibold text-gray-900 tabular-nums dark:text-white">{value}</p>
    </div>
  );
}
...
function ClusterCard({ cluster, reports }: { cluster: string; reports: EventReport[] }) {
  ...
  const totals = sumPopulationCountsByCategory(reports);

  const totalCasualties = reports.reduce((s, r) => s + r.casualties.length, 0);
  const totalMissing = reports.reduce((s, r) => s + r.missing_persons.length, 0);
  const totalAffected = totals.reduce((sum, t) => sum + t.total, 0);
    ...
    <div className="shadow-theme-md overflow-hidden rounded-xl border border-gray-200 bg-white dark:border-white/5 dark:bg-white/3">
      ...
      <div className="px-5 py-4">
        <div className="grid grid-cols-2 gap-x-8 divide-y divide-gray-100 sm:grid-cols-3 dark:divide-white/5">
          {totals.map((t) => (
            <HeadcountRow key={t.id} label={t.name} value={t.total} />
          ))}
        </div>
      </div>
            ...
            <div className="mb-3 grid grid-cols-3 gap-2 sm:grid-cols-4 lg:grid-cols-6">
              {r.population_counts
                ...
                .map((pc) => (
                  ...
                  >
                    <p className="text-xs text-gray-400">{pc.category.name}</p>
                  ...
                  <div className="border-error-200 bg-error-50 dark:border-error-500/20 dark:bg-error-500/10 rounded-lg border p-3">
                    ...
                    <div className="space-y-1">
                      {r.casualties.map((c) => (
                        ...
                        >
                          <span className="text-error-700 dark:text-white">{c.name || '—'}</span>
                          <span className="text-error-700 dark:text-white">
...
function UnitBreakdownTabbed({
  ...
  reports.forEach((r) => {
    ...
    const affected = totalPopulationCount(r.population_counts);
